...
window.onload = function () {
    ...
    // через каждую секунду часы перерисовываются 
...
function clock() { 
    ...
    // рисуя в точке 0,0 фактически рисуем в точке 75,75 
    ctx.translate(75, 75);
  
    // при рисовании линии в 100px фактически рисуем линию в 40px 
    ctx.scale(0.4, 0.4);
    ...
    // рисуем минутные метки для каждой минуты 
    for (var i = 0; i < 60; i++) {    
        // кроме тех, что совпадут с часами   
        if (i % 5 != 0) {      
            // перемещаем курсор     
            ctx.moveTo(117, 0);        
            // рисуем черточку 3px         
    ...
    // сдвигаем курсор несколько назад, 4тобы было похоже на стрелку
    ...
    // рисуем внешнюю окружность шириной 14px 
    ctx.lineWidth = 14;
    // синим цветом  
    ctx.strokeStyle = "#325FA2";
    ctx.beginPath(); 
    // рисуем окружность, отступающую от центра на 142px
